...
public class DiffMatchPatch {
     ...
     */
    public String diff_prettyHtml_BK1(List<Diff> diffs) throws IOException {
        String del = "";
        String inser = "";
        String change = "";
        ...
        for (Diff aDiff : diffs) {
            ...
            switch (aDiff.operation) {
                case INSERT:
                    if(!del.isEmpty()){
                        String [] strInser = text.split("&nbsp;");
                        if(strInser.length > 0){
                            if(strInser.length >= 2) {
                                change = "<font style=\"background-color: lightgreen\" color=\"black\">" + strInser[0].trim() + "</font>";
                                inser = "<font style=\"background-color: yellow\" color=\"black\">" + strInser[1].trim() + "</font>";
                            }else{
                                change = "<font style=\"background-color: lightgreen\" color=\"black\">" + strInser[0].trim() + "</font>";
                            }
                        }
                        String strhtml = html.toString().replace("delete",del + " ").replace("insert",change + " " + inser + " ");
                        html = new StringBuilder();
                        html.append(strhtml);
                    }else{
                        inser = "<font style=\"background-color: yellow\" color=\"black\">"+ text + "</font>";
                        String strhtml = html.toString().replace("delete",addMoreBrTag(countWordsUsingSplit(inser), del)).replace("insert",inser.trim());
                        html = new StringBuilder();
                        html.append(strhtml);
                    }
                    del = "";
                    ...
                    break;
                case DELETE:
                    del = "<font style=\"background-color: lightgreen\" color=\"black\">" + text + "</font>";
                    break;
                case EQUAL:
                    if(!text.startsWith("\"")) {
                        if(text.startsWith("&gt;")){
                        ...
                        html.append("<tr>");
                            html.append("<td><span>").append(text).append("delete").append("</span></td>");
                            html.append("<td><span>").append(text).append("insert").append("</span></td>");
                        html.append("</tr>");
                    }
                    break;
            }
        ...
        WriteToFile(html.toString(), new SimpleDateFormat("yyyyMMdd_HHmmss").format(Calendar.getInstance().getTime()) + "_XmlCompareResult.html");
        return html.toString();
    }

    public String diff_prettyHtml_BK2(List<Diff> diffs) throws IOException {
        String del = "";
        StringBuilder html = new StringBuilder();
        html.append("<html><head><title>Selenium Test </title></head>");
        html.append("<body>");
        ...
        for (Diff aDiff : diffs) {
            ...
            switch (aDiff.operation) {
                case INSERT:
                    html.append("<tr>");
                    html.append("<td><span>").append("<ins style=\"background:#e6ffe6;\">").append(del)
                            .append("</ins></span></td>");
                    html.append("<td><span>").append("<del style=\"background:#e6ffe6;\">").append(text)
                            .append("</del></span></td>");
                    html.append("</tr>");
                    del = "";
                    break;
                ...
                case EQUAL:
                    html.append("<tr>");
                        html.append("<td><span>").append(text).append("</span></td>");
                        html.append("<td><span>").append(text).append("</span></td>");
                    html.append("</tr>");
                    break;
            }
     ...
     * @return HTML representation.
     */
    public String diff_prettyHtml_First(List<Diff> diffs) throws IOException {
        StringBuilder html = new StringBuilder();
        for (Diff aDiff : diffs) {
            String text = aDiff.text.replace("&", "&amp;").replace("<", "&lt;")
                    .replace(">", "&gt;").replace("\n", "&para;<br>");
        ...
        WriteToFile(html.toString(), "TestXmlCompareResult.html");
    ...
    public String diff_prettyHtml(List<Diff> diffs, String actualFile, String expectedFile) throws IOException {
        ...
        for (Diff aDiff : diffs) {
            ...
            switch (aDiff.operation) {
                case INSERT:
                    inser =  "<span style=\"background:#e6ffe6;\">" + text + "</span>";;
                    break;
                case DELETE:
                    del = "<span style=\"background:#ffe6e6;\">" + text + "</span>";
                    break;
                case EQUAL:
                    equal1 += del + text;
                    equal2 += inser + text;
        ...
        html.append("<tr>");
            html.append("<td><span>").append(equal1).append("</span></td>");
            html.append("<td><span>").append(equal2).append("</span></td>");
        html.append("</tr>");
        html.append("</table></body></html>");


        WriteToFile(html.toString(), "_XmlCompareResult.html");
        return html.toString();
    }
    ...
    private static int countWordsUsingSplit(String input) {
        int counter = 0;
        return input.split("<br>").length - 2;
    }

    private static String addMoreBrTag(int count, String str){
        if(count > 0) {
    ...
    private static void WriteToFile(String fileContent, String fileName) throws IOException {
        String projectPath =  System.getProperty("user.home")+"\\Desktop\\QA_Project\\";
        if(!new File(projectPath).exists())
            Common.createUserDir(projectPath);
        String tempFile = projectPath + File.separator + System.currentTimeMillis()/1000 + fileName;
