...
public class GetCourseDetailsFromKyna extends ApisBaseTest {
    ...
    public void getCourseDetail() {
        ...
        Response response =
                RestAssured.given().config(RestAssured.config().sslConfig(new SSLConfig().allowAllHostnames()))
                        ...
                        .when()
                        .get(Config.getProp("kynabizApis") + Config.getProp("getCourseDetailEndPoint"));

        //assert response
        Log.info(response.getBody().asString());
        Assert.assertEquals(response.getStatusCode(), 200);
        JsonHelper.compareTwoJsonObject(Config.getProp("kynaCourseDetail"),response.getBody().asString());
