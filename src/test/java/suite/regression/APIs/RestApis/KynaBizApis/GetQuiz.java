...
public class GetQuiz extends ApisBaseTest {
    ...
    public void getQuiz() {
        ...
        Response response =
                RestAssured.given().config(RestAssured.config().sslConfig(new SSLConfig().allowAllHostnames()))
                        ...
                        .when()
                        .get(Config.getProp("kynabizApis") + Config.getProp("getQuizEndPoint"));

        //assert response
        Log.info(response.getBody().asString());
        Assert.assertEquals(response.getStatusCode(), 200);
        JsonHelper.compareTwoJsonObject(Config.getProp("quiz"),response.getBody().asString());
